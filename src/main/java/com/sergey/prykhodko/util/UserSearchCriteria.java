package com.sergey.prykhodko.util;

import com.sergey.prykhodko.model.User;

import java.io.Serializable;

public class UserSearchCriteria implements Serializable{
    private Integer id;
    private String login;
    private String email;
    private String firstName;
    private String lastName;
    
    
	public UserSearchCriteria() {
	}	
	
	public boolean isEmpty() {
		return (id == null && email == null && firstName == null && lastName == null);
	}

	public void setId(Integer id) {
		this.id = id;
	}

	public String getLogin() {
		return login;
	}

	public void setLogin(String login) {
		this.login = login;
	}

	public Integer getId() {
		return id;
	}

	public void setId(int id) {
		this.id = id;
	}

	public String getEmail() {
		return email;
	}

	public void setEmail(String email) {
		this.email = email;
	}

	public String getFirstName() {
		return firstName;
	}

	public void setFirstName(String firstName) {
		this.firstName = firstName;
	}

	public String getLastName() {
		return lastName;
	}

	public void setLastName(String lastName) {
		this.lastName = lastName;
	}

	public boolean accept(User user) {
		return acceptId(user.getId())
				&& acceptLogin(user.getLogin())
				&& acceptEmail(user.getEmail())
				&& acceptFirstName(user.getFirstName())
				&& acceptLastName(user.getLastName());
	}

	private boolean acceptId(Integer id) {
		return this.id == null || this.id.equals(id);
	}

	private boolean acceptLogin(String login) {
		return this.login == null || this.login.equals(login);
	}

	private boolean acceptEmail(String email) {
		return this.email == null || this.email.equals(email);
	}

	private boolean acceptFirstName(String firstName) {
		return this.firstName == null || this.firstName.equals(firstName);
	}

	private boolean acceptLastName(String lastName) {
		return this.lastName == null || this.lastName.equals(lastName);
	}

	@Override
	public int hashCode() {
		final int prime = 31;
		int result = 1;
		result = prime * result + ((email == null) ? 0 : email.hashCode());
		result = prime * result + ((firstName == null) ? 0 : firstName.hashCode());
		result = prime * result + id;
		result = prime * result + ((lastName == null) ? 0 : lastName.hashCode());
		return result;
	}

	@Override
	public boolean equals(Object obj) {
		if (this == obj)
			return true;
		if (obj == null)
			return false;
		if (getClass() != obj.getClass())
			return false;
		UserSearchCriteria other = (UserSearchCriteria) obj;
		if (email == null) {
			if (other.email != null)
				return false;
		} else if (!email.equals(other.email))
			return false;
		if (firstName == null) {
			if (other.firstName != null)
				return false;
		} else if (!firstName.equals(other.firstName))
			return false;
		if (id.equals(other.id))
			return false;
		if (lastName == null) {
			if (other.lastName != null)
				return false;
		} else if (!lastName.equals(other.lastName))
			return false;
		return true;
	}

	@Override
	public String toString() {
		return "UserSearchCriteria [id=" + id + ", email=" + email + ", firstName=" + firstName + ", lastName="
				+ lastName + "]";
	}

	
    
}
